...
const EditarClienteModal = ({ isOpen, onRequestClose, idCliente, detalhes, mostrarMensagemSucesso, mostrarMensagemSemAlteracao }) => {
    const { editarClienteRequest, uploadImagemClienteRequest } = useAdminProvider();
    ...
    const validarResponse = (response) => {
        if(response === "Cliente alterado com sucesso")
    ...
    const toggleCheckbox = () => {
        if(userData.arquivado) {
            setUserData({ ...userData, arquivado: false })
            setEditBody({ ...editBody, arquivado: false })
        } else {
            setUserData({ ...userData, arquivado: true });
            setEditBody({ ...editBody, arquivado: true });
        }
    }
    ...
    const handleSubmit = async () => {
        ...
        if(editBody.nome || editBody.cnpj || editBody.logix || editBody.imagem_url || editBody.arquivado === true || editBody.arquivado === false) {
            const response = await editarClienteRequest(idCliente, editBody);
    ...
    return (
        <ModalContainer isOpen={isOpen} onRequestClose={closeModal}>
            <div className="formUser__container">
                ...
                {!editBody || !editBody.imagem_url 
                    ? <UserPhoto imagem_url={detalhes.imagem_url} max_size={169} path='cliente/' />
                ...
                <form action="" className='formUser__form' onSubmit={e => e.preventDefault()}>
                    ...
                    <div className="formUser__field">
                        ...
                        <input id="formUser__cnpj" 
                            type="text" 
                            className='formUser__input'
                            placeholder="0000000000/0000-0" 
                            value={cnpjMask(userData.cnpj)}
                            onChange={(e) => {setUserData({ ...userData, cnpj: e.target.value }); setEditBody({ ...editBody, cnpj: e.target.value })}} />
                    ...
                    <div className="formUser__field">
                        ...
                        <input id="formUser__logix" 
                            type="text" 
                            className='formUser__input'
                            placeholder="000000000000" 
                            value={userData.logix}
                            onChange={(e) => {setUserData({ ...userData, logix: e.target.value }); setEditBody({ ...editBody, logix: e.target.value })}} />
